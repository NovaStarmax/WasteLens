...
@router.get(
    ...
    description="Returns model evaluation metrics (accuracy, F1, precision, recall per class). Requires Bearer JWT token.",
    responses={
        401: {"description": "Token expired or invalid"},
        404: {"description": "Evaluation report not found"},
    },
)
async def get_evaluation(_: dict = Depends(verify_token)) -> dict:
...
@router.get(
    ...
    description="Returns the confusion matrix as a PNG image. Requires Bearer JWT token.",
    responses={
        401: {"description": "Token expired or invalid"},
        404: {"description": "Confusion matrix not found"},
    },
)
async def get_confusion_matrix(_: dict = Depends(verify_token)) -> FileResponse:
